...
const SiteI_ReelFake = ({ onHoverLink, isExplaining }) => {
    ...
    <div className="min-h-full bg-black text-white font-sans overflow-hidden flex flex-col">
      ...
      <header className="border-b border-gray-800 bg-black sticky top-0 z-10 px-4 h-16 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-gradient-to-tr from-purple-600 to-orange-500 rounded-lg flex items-center justify-center">
            <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 20 20"><path d="M4 4l12 6-12 6z"/></svg>
          </div>
          <span className="text-xl font-bold tracking-tight">Reel</span>
        </div>
        <div className="hidden md:flex relative w-96">
          <input 
            type="text" 
            placeholder="Tìm kiếm video..." 
            className="w-full bg-gray-900 border border-gray-700 rounded-full px-5 py-2.5 text-sm focus:outline-none focus:border-purple-500 text-white"
          />
        ...
        <div className="flex items-center gap-4">
          ...
          <button className="bg-purple-600 hover:bg-purple-700 text-white font-bold px-6 py-1.5 rounded text-sm transition-colors">Đăng nhập</button>
        </div>
      </header>
      ...
      <main className="flex-1 flex max-w-5xl mx-auto w-full pt-6 relative">
        ...
        <aside className="w-60 hidden md:block flex-shrink-0 pr-4 space-y-6">
          ...
          <div className="border-t border-gray-800 pt-4 text-xs text-gray-500 space-y-2">
            ...
            <button className="w-full border border-purple-600 text-purple-500 font-bold py-2 rounded hover:bg-purple-900/30 mt-2 transition-colors">Đăng nhập</button>
          </div>
        </aside>
        ...
        <div className="flex-1 flex justify-center pb-20">
          <div className="w-[350px] bg-gray-900 rounded-xl overflow-hidden relative shadow-2xl h-[600px]">
            ...
            <div className="absolute bottom-4 left-4 right-16">
              <h3 className="font-bold mb-1">@reel_creator</h3>
              <p className="text-sm">Sự kiện tặng xu cực khủng năm 2026! Nhấn link để nhận ngay 5000 Xu. #reel #xuhuong</p>
            </div>
            ...
            <div className="absolute bottom-4 right-2 flex flex-col gap-4 items-center">
              ...
              <div className="flex flex-col items-center gap-1"><svg className="w-8 h-8 text-white hover:text-purple-500 transition-colors cursor-pointer" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clipRule="evenodd"/></svg><span className="text-xs">125K</span></div>
              <div className="flex flex-col items-center gap-1"><svg className="w-8 h-8 text-white hover:text-gray-300 transition-colors cursor-pointer" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M18 10c0 3.866-3.582 7-8 7a8.841 8.841 0 01-4.083-.98L2 17l1.338-3.123C2.493 12.767 2 11.434 2 10c0-3.866 3.582-7 8-7s8 3.134 8 7zM7 9H5v2h2V9zm8 0h-2v2h2V9zM9 9h2v2H9V9z" clipRule="evenodd"/></svg><span className="text-xs">4023</span></div>
              <div className="flex flex-col items-center gap-1"><svg className="w-8 h-8 text-white hover:text-gray-300 transition-colors cursor-pointer" fill="currentColor" viewBox="0 0 20 20"><path d="M15 8a3 3 0 10-2.977-2.63l-4.94 2.47a3 3 0 100 4.319l4.94 2.47a3 3 0 10.895-1.789l-4.94-2.47a3.027 3.027 0 000-.74l4.94-2.47C13.456 7.68 14.19 8 15 8z"/></svg><span className="text-xs">Share</span></div>
            </div>
          </div>
        </div>
      </main>
      ...
      {showPopup && (
        <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black/80 transition-all ${isExplaining ? 'bg-transparent' : ''}`}>
          <div className={`bg-[#121212] border border-gray-800 w-full max-w-sm rounded-xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 relative ${isExplaining ? 'ring-4 ring-red-500 scale-105 shadow-[0_0_40px_rgba(239,68,68,0.7)]' : ''}`}>
            ...
            {/* Header */}
            <div className="relative h-24 bg-gradient-to-r from-purple-600 to-orange-500 flex items-center justify-center overflow-hidden">
              <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-30"></div>
              <h2 className="text-2xl font-black text-white relative z-10 italic">REEL REWARDS</h2>
              <button 
            ...
            <div className="p-6 flex flex-col items-center text-center">
              ...
              <p className="text-gray-400 text-sm mb-6">Bạn đã được chọn ngẫu nhiên để nhận <b className="text-yellow-400">5,000 Xu Reel</b> hoàn toàn miễn phí. Đăng nhập ngay để nhận vào ví.</p>
              
              <div className="w-full space-y-3">
                <input 
                  type="text" 
                  placeholder="Reel ID, Email hoặc Số điện thoại" 
                  className="w-full bg-gray-800 border border-gray-700 text-white rounded p-3 text-sm focus:outline-none focus:border-purple-500"
                />
                <input 
                  type="password" 
                ...
                />
                <button 
                  onMouseEnter={() => onHoverLink('http://hacker-api.net/steal/reel-credentials')}
                  onMouseLeave={() => onHoverLink('')}
                  className="w-full bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 rounded transition-colors shadow-lg shadow-purple-500/20"
                >
                  XÁC NHẬN NHẬN XU
                </button>
              </div>
            ...
            <div className="bg-gray-900 py-3 text-center text-xs text-gray-500 border-t border-gray-800">
              Chương trình được tài trợ bởi Reel Official.
            </div>
          </div>
        </div>
      )}
